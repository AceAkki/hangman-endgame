...
function App() {
  ...
  const time = new Date();
  time.setSeconds(time.getSeconds() + (currentWord.length * 20));

  let wrongGuessCount = guessLetter.filter(
    (letter) => !currentWord.includes(letter)
  ).length;

  const isGameWon = Array.from(currentWord).every((char) =>
    guessLetter.includes(char)
  );
  ...
  function loadPuzzle() {
    let puzzleWord = Array.from(currentWord).map((char) => {
      return (
        <span
          className={clsx("puzzle-char", {
            hidden: !guessLetter.includes(char) && !isGameOver,
            reveal: !guessLetter.includes(char) && isGameOver,
          })}
          key={nanoid()}
        >
          {" "}
  ...
  function guessChar(letter) {
    ...
    setKeyboardKeys((oldkeys) =>
      oldkeys.map((obj) => {
        return obj.txt === letter
          ? {
              ...obj,
              class: {
                "correct-guess": currentWord.includes(letter),
                "wrong-guess": !currentWord.includes(letter),
              },
            }
          : obj;
      })
    );
    setGuessLetter((oldword) =>
      oldword.includes(letter) ? oldword : [...oldword, letter]
    );
     console.log(time.getSeconds(), time.getSeconds() + (currentWord.length * 20) )
  }
  ...
  function renderGameStatus() {
    return isGameOver
      ? isGameWon
        ? "You won ! Well Done. You saved the programming world."
        : "You lost the game, Better start learning Assembly"
      : wrongGuessCount > 0
      ? `${languages[wrongGuessCount - 1].name} is lost to the world`
      : null;
  }
  ...
  let langChips = languages.map((lang, index) => {
    ...
    return (
      <div
        key={lang.name}
        className={clsx("lang", { lost: index < wrongGuessCount })}
        style={styles}
        aria-description={lang.text}
        onMouseEnter={flipEnter}
        onMouseLeave={() => flipLeave(lang)}
      >
        {" "}
        {lang.name}{" "}
      </div>
  ...
  return (
    <>
      <Header />
      <main>
        {isGameWon ? <Confetti width={width} height={height} /> : null}
        <section
          className={clsx("status-text", {
            won: isGameWon,
            lost: isGameLost,
            "lang-status": !isGameOver && wrongGuessCount > 0,
          })}
          aria-live="polite"
          role="status"
        >
        ...
        <section className="languages-chips">{langChips}</section>
        <section className="puzzle-txt-wrap">{loadPuzzle()}</section>
        ...
        <section className="sr-only" aria-live="polite" role="status">
          <p>
            {currentWord.includes(lastGuessLetter)
              ...
              : `Wrong! the letter ${lastGuessLetter} is not in the word`}
        ...
        <section className="keyboard-wrap">{createKeyboard()}</section>
